function [t_sim_elapsed, t_render_elapsed] = simulate_speakers_upscale(varargin)
...
%                         rendering video.
%
% (unchanged header omitted for brevity — same as your original)
%
% NEW OPTIONS:
%   'src_spacing' : spacing between two identical sources (m). Default 0 (single source).
%   'alpha'       : rotation angle (degrees) of each line source relative to +y direction.
%
% ... rest of header ...
...
% NEW options for stereo/two sources
addParameter(p, 'src_spacing', 0);   % meters, distance between two sources (0 -> single source)
addParameter(p, 'alpha', 0);         % degrees, rotation of each line source relative to +y (vertical)
...
%% --- Source definition ---
% A line-like source (a mask along a rotated line at chosen x positions)
source.p_mask = zeros(Nx, Ny);

% compute centerline indices
cx_grid_mid = round(Nx / 2);     % grid midline (index)
cy = round(Ny / 2);              % center y index (mid-plane)

% convert requested src_length into number of sample points for rasterization
half_len_cells = round((opt.src_length / dx) / 2);
n_line_samples = max(3, 2 * half_len_cells + 1);  % at least 3 samples along the line

% Determine whether to create two symmetric sources or single one
if opt.src_spacing > 0
    % two sources symmetric about y-axis (domain midline)
    % centers (in meters) are at x = +/- d/2 relative to domain center (midline)
    d = opt.src_spacing;
    alpha_rad = deg2rad(opt.alpha);
    
    % compute x-index offsets for each source center
    half_d_cells = (d / 2) / dx;   % can be fractional; we'll add to center index
    cx1 = cx_grid_mid - half_d_cells;
    cx2 = cx_grid_mid + half_d_cells;
    
    % rasterize a rotated line at (cx1, cy) and (cx2, cy)
    mask1 = make_rotated_line_mask(cx1, cy, opt.src_length, dx, Nx, Ny, n_line_samples, alpha_rad);
    mask2 = make_rotated_line_mask(cx2, cy, opt.src_length, dx, Nx, Ny, n_line_samples, alpha_rad);
    
    source.p_mask = double(mask1 | mask2);
    
    % For directional sampling, choose ring center between the two sources (domain midline)
    cx = cx_grid_mid;
else
    % single source (preserve original logic: use cx_frac)
    ...
    source.p_mask(cx, yind) = 1;      % place the line source mask (vertical)
end
...
%% ----------------------- New helper: rasterize rotated line --------------------
function mask = make_rotated_line_mask(cx_idx_cont, cy_idx, src_length_m, dx, Nx, Ny, n_samples, alpha_rad)
    % cx_idx_cont: can be fractional x-index (center), cy_idx integer
    % src_length_m: physical length in meters
    % dx: grid spacing (m)
    % n_samples: number of sampling points along the line
    % alpha_rad: rotation angle in radians, measured from +y (vertical) toward +x
    %
    % returns mask Nx-by-Ny logical where points on the rotated line are 1
    
    % param s runs from -L/2 .. +L/2 (meters)
    s = linspace(-src_length_m/2, src_length_m/2, n_samples);
    % direction vector for a line rotated by alpha relative to +y:
    % at alpha=0 -> direction along +y: (dx_idx_x = 0, dx_idx_y = +1)
    dir_x = sin(alpha_rad);   % how much x changes per meter along line
    dir_y = cos(alpha_rad);   % how much y changes per meter along line
    
    % compute continuous coordinates in indices
    x_cont = cx_idx_cont + (s / dx) * dir_x;
    y_cont = cy_idx + (s / dx) * dir_y;
    
    % round to nearest grid indices and clamp
    xi = round(x_cont); yi = round(y_cont);
    xi = max(1, min(Nx, xi));
    yi = max(1, min(Ny, yi));
    ...
    % Optionally fill small holes along the rasterized line (connectivity)
    % use morphological bridging (only if Image Processing Toolbox available)
    try
        mask = imdilate(mask, strel('line',3,round(rad2deg(alpha_rad))));
    catch
        % ignore if imdilate or strel not available
    end
end
